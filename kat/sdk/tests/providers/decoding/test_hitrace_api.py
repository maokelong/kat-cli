...
class HitraceApiContractTests(unittest.TestCase):
    def test_public_surface_is_small_and_report_is_immutable(self) -> None:
        ...
        self.assertEqual(kat.providers.decoding.__all__, ("hitrace", "text_ftrace"))
        self.assertEqual(
            hitrace.__all__,
            (
                "decode",
        ...
        self.assertFalse(hasattr(kat.providers.decoding, "decode"))
        self.assertFalse(hasattr(kat.providers.decoding, "DecodeReport"))
        self.assertFalse(hasattr(kat.providers.decoding, "DecodeError"))
    ...
    def test_distribution_version_is_normalized_from_cargo_release_version(self) -> None:
        ...
        self.assertEqual(
            _datasource_distribution().version,
    ...
    def test_native_decode_releases_the_gil(self) -> None:
        ...
        importlib.import_module("kat.providers._native")
    ...
    def test_distribution_owns_source_without_other_kat_wheel_dependency(self) -> None:
        distribution = _datasource_distribution()

        self.assertFalse(any(requirement.startswith("kat-") for requirement in distribution.requires or ()))
        self.assertIsNone(importlib.util.find_spec("kat_sdk"))
        self.assertEqual(
...
def _datasource_distribution():
    # 同时验证内部原生构建输入与最终 SDK，按 RECORD 查找真实文件所有者。
    from kat.providers.decoding import hitrace

    entry = "kat/providers/decoding/hitrace.py"
    owners = [distribution for distribution in importlib.metadata.distributions()
              if entry in {str(path) for path in distribution.files or ()}]
    assert len(owners) == 1, owners
    assert owners[0].locate_file(entry).resolve() == pathlib.Path(hitrace.__file__).resolve()
    return owners[0]

...
def read_public_provider_guide(reference: str) -> str:
    ...
    if relative.is_absolute() or ".." in relative.parts or not relative.parts or relative.suffix != ".md":
        raise ValueError("Public Provider guide must be relative to SDK knowledge/")
    target = files("kat.knowledge").joinpath(*relative.parts)
    if not target.is_file():
        raise ValueError(f"Public Provider guide is missing from the installation: {reference}")

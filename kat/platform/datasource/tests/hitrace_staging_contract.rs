...
#[test]
fn staging_writes_long_term_clock_and_switch_facts_in_source_order() {
    ...
    kat_datasource::stage_hitrace(&source, &dataset, |_| Ok(())).expect("complete capture stages");
    let tables = hitrace_table_names(&dataset);
    assert_eq!(tables, ["clock_domain", "clock_snapshot", "sched_switch"]);

    let mut rows = Vec::new();
    for batch in batches(&hitrace_table_path(&dataset, "sched_switch")) {
        let domains = batch
    ...
    assert_eq!(
        batches(&hitrace_table_path(&dataset, "clock_snapshot"))
            .iter()
...
#[test]
fn staging_batches_switches_without_deriving_sequence_from_parquet_order() {
    let root = tempdir().expect("tempdir");
    let source = root.path().join("capture.htrace");
    let dataset = root.path().join("dataset");
    ...
    kat_datasource::stage_hitrace(&source, &dataset, |_| Ok(())).expect("capture stages");
    let batches = batches(&hitrace_table_path(&dataset, "sched_switch"));
    assert_eq!(
...
#[test]
fn staging_batches_clock_snapshots_without_changing_source_order() {
    let root = tempdir().expect("tempdir");
    ...
    kat_datasource::stage_hitrace(&source, &dataset, |_| Ok(())).expect("clock snapshots stage");

    let batches = batches(&hitrace_table_path(&dataset, "clock_snapshot"));
...
fn clock_and_thread_continuity_damage_fail_before_target_mutation() {
    for (events, expected) in [
        ...
        let error = kat_datasource::stage_hitrace(&source, &dataset, |_| Ok(()))
...
#[test]
fn every_loss_evidence_rejects_complete_staging() {
...
fn capture_damage_is_irrelevant_without_supported_ftrace_events() {
    ...
    kat_datasource::stage_hitrace(&source, &dataset, |_| Ok(()))
...
fn missing_ftrace_clock_is_allowed_without_supported_events() {
    ...
    kat_datasource::stage_hitrace(&source, root.path().join("dataset"), |_| Ok(()))
...
fn capture_requires_one_complete_end_snapshot_and_one_clock() {
    ...
    for (result, expected) in cases {
        ...
        let source = root.path().join("capture.htrace");
        write_fixture(&source, result);
        let error = kat_datasource::stage_hitrace(&source, root.path().join("dataset"), |_| Ok(()))
...
fn real_openharmony_capture_smoke() {
    ...
    let root = tempdir().expect("tempdir");
    let staged = kat_datasource::stage_hitrace(&source, root.path().join("dataset"), |_| Ok(()))
        .expect("real OpenHarmony capture stages");

    assert!(
        staged
            .table_names()
            .iter()
            .any(|table| table == "sched_switch")
    );
}

fn hitrace_table_names(staging: &Path) -> Vec<String> {
    let mut names = fs::read_dir(staging.join("tables"))
        .expect("staged tables directory reads")
        .map(|entry| {
            entry
                .expect("staged table entry reads")
                .path()
                .file_stem()
                .and_then(|name| name.to_str())
                .expect("staged table name is Unicode")
                .to_owned()
        })
        .collect::<Vec<_>>();
    names.sort();
...
fn hitrace_table_path(staging: &Path, table: &str) -> PathBuf {
    staging.join("tables").join(format!("{table}.parquet"))
}

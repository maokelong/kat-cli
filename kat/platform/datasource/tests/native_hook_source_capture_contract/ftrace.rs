...
fn ftrace_branch_claims_only_its_two_exact_routes() {
    ...
    let mut capture = NativeHookCaptureFixture::new(protobuf_source::BufferOptions::new(2))
...
-> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        .await,
        json!([
            {"kind": "data", "envelope_name": "ftrace-plugin", "_kat_parent_row_id": 0},
            {"kind": "config", "envelope_name": "ftrace-plugin_config", "_kat_parent_row_id": 1},
        ])
    );
    assert_eq!(
        query_json(
            &context,
            "select config.parse_mode as enum_number, definition.enum_symbol \
             from trace_plugin_config config \
             join protobuf_enum_symbol definition \
               on definition.origin_table = 'trace_plugin_config' \
              and definition.origin_field_path = 'parse_mode' \
              and definition.enum_number = config.parse_mode",
        )
        .await,
        json!([{"enum_number": 2, "enum_symbol": "RAW_DATA"}])
    );

...
impl ActiveRelation {
    pub(super) fn new(
        ...
        let writer = relations
            .begin(spec.name, spec.schema.clone())
            .with_context(|| {
                format!(
                    "failed to open staged protobuf Source relation {:?}",
                    spec.name
                )
            })?;
        ...
        T: EstimatedRow,
    {
        ...
        self.builder.push(row).with_context(|| {
            format!(
                "failed to serialize protobuf Source row for relation {:?}",
                self.spec.name
            )
        })?;
    ...
    fn flush(&mut self) -> Result<()> {
        ...
        let batch = self.builder.to_record_batch().with_context(|| {
            format!(
                "failed to build protobuf Source batch for relation {:?}",
                self.spec.name
            )
        })?;
        self.writer.write(&batch).with_context(|| {
            format!(
                "failed to write staged protobuf Source relation {:?}",
                self.spec.name
            )
        })?;
    ...
    pub(super) fn finish(mut self) -> Result<()> {
        ...
        self.writer.finish().with_context(|| {
            format!(
                "failed to finish staged protobuf Source relation {:?}",
                self.spec.name
            )
        })
    }

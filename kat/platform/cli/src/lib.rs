...
pub fn run() -> ExitCode {
    ...
    match cli.operation {
        ...
        Operation::Test(arguments) => response::publish(test_pack(arguments)),
    }
}

fn test_pack(arguments: TestArgs) -> response::PreparedResponse<TestPackResult> {
    ...
    let mut log = match OperationLog::create_test(&data_home, &token, |file| {
        writeln!(
            file,
            "operation: kat test\npack: {:?}\ntests: {:?}",
            arguments.pack, arguments.tests
        )
    }) {
        Ok(log) => log,
    ...
    let Some(pack) = discovered.get(&arguments.pack) else {
        return finish_test_pack_failure(
            log,
            TestPackOperationError::UnknownPack {
                name: arguments.pack,
            },
        );
    };
        ...
        .find(|selector| !valid_test_selector(selector))
    {
        return finish_test_pack_failure(
            ...
            TestPackOperationError::InvalidSelector {
                selector: diagnostic_safe_argument(selector),
            },
        );
    }
    ...
    let report = match completed_test_report(&report_path) {
        ...
        Err(error) => {
            let log_path = outcome
                .as_ref()
                .err()
                .and_then(|error| error.log_path())
                .or_else(|| {
                    outcome.as_ref().ok().map(|outcome| match outcome {
                        workflow_runtime::TestPackOutcome::Success { log_path, .. }
                        | workflow_runtime::TestPackOutcome::Failure { log_path, .. } => {
                            log_path.clone()
                        }
                    })
                });
            return response::prepare_test_cli_failure(miette::Report::new(error), log_path, None);
        }
    };
    match outcome {
        ...
        Ok(workflow_runtime::TestPackOutcome::Failure {
            diagnostic,
            log_path,
        }) => response::prepare_test_runtime_failure(diagnostic, log_path, report),
        Err(error) => {
            let log_path = error.log_path();
            response::prepare_test_cli_failure(miette::Report::new(error), log_path, report)
        }
    }
}
...
enum TestPackOperationError {
    ...
    #[diagnostic(help("Use a pytest node ID whose path begins with tests/ and stays in the PACK"))]
...
mod tests {
    ...
    #[test]
    fn test_selector_validation_only_enforces_the_pack_path_envelope() {
        assert!(valid_test_selector("tests/./test_flow.py::test_case"));
        assert!(valid_test_selector("tests//test_flow.py"));
        assert!(valid_test_selector(r"tests/foo\bar.py::test_case"));
        assert!(!valid_test_selector("tests/../secret.py"));
        assert!(!valid_test_selector("/tests/test_flow.py"));
        assert!(!valid_test_selector(r"C:\tests\test_flow.py"));
    }

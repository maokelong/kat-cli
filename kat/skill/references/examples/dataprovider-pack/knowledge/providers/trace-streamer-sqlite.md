...
关闭后通过 `dp.Table.from_rows()` 构造不可变 eager `dp.Table`。连接使用 `mode=ro`、
`PRAGMA query_only` 和 SQLite authorizer，仅允许只读查询。调用方必须用
`pyarrow.Schema` 按 SQL 投影顺序声明精确物理类型与 nullability；实际列名或顺序不一致会失败。

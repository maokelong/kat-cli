...
源码路径：`kat/sdk/libraries/demo/greeting.py`。完整类型、异常与示例来自当前安装 SDK 构建生成的 API 参考。按 [Python 依赖管理](../../python-packages.md) 定位相邻 Python，再读取：

```text
<bundled-python> -I -B -c "from importlib.resources import files; print(files('kat_sdk').joinpath('knowledge/libraries/demo/greeting.api.md').read_text(encoding='utf-8'))"
```

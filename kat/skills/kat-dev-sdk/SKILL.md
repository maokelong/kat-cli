...
2. 读取 [SDK 开发指南](references/sdk-development.md)，选择 Provider、Workflow 或公共库分支。三类能力开发前均执行 [开发前复用检查](../kat/references/helpers/index.md#新开发前的复用检查)，检查已有同类实现；Workflow/Provider 还须检查公共函数能否支撑构建。Workflow 和 helpers 按领域分目录，只实现已确认的缺口，使用框架公开 API。

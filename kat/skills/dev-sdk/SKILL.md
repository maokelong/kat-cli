...
2. 读取 [SDK 开发指南](references/sdk-development.md)，选择 Provider、Workflow 或公共库分支。Workflow 和公共库均按领域分目录，使用框架公开 API。
3. 同步实现、类型注解、docstring、使用知识和导航。完成标准是实际 wheel 中包含所需源码与可读文档，名称和导入路径一致。
4. 构建独立 wheel，在测试部署中验证知识读取、真实使用及升级。复验未安装和卸载 SDK 后，原有框架与不依赖 SDK 的 PACK 仍可用。

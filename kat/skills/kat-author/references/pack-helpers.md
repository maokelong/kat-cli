...
执行 [开发前复用检查](../../kat/references/reuse-check.md)，同时检查目标 PACK 已有的 `helpers/`、文档和实际调用点，明确函数服务的 Workflow/Provider。对比用途、输入输出、语义及限制；满足需求时直接复用，有缺口时说明检查范围与缺口，只实现已授权的必要部分。检查失败时停止该函数开发。

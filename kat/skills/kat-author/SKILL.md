...
1. 读取 [PACK 创作与维护流程](references/pack-authoring-flow.md)；Workflow、Provider、公共函数开发前均执行 [开发前复用检查](../kat/references/helpers/index.md#新开发前的复用检查)，核对 SDK 与当前 PACK 已有同类实现，开发 Workflow/Provider 时同时检查公共函数能否支撑构建。新增或提取 PACK 内函数库时读取 [领域公共库开发](references/pack-helpers.md)，完成实现、文档及测试。

...
def test_uses_source_sequence_and_keeps_honest_boundaries(kat_run):
    output = kat_run(workflow="thread-cpu-time", dataset="semantics")

    assert output["thread_cpu_time_by_cpu"].to_pylist() == [
        {
            "thread_id": 10,
            "thread_name": "worker-old",
            "cpu": 0,
            "observed_cpu_time_ns": 50,
        },
        {
            "thread_id": 20,
            "thread_name": "mover",
        ...
        {
            "thread_id": 20,
            "thread_name": "renamed-mover",
            "cpu": 1,
            "observed_cpu_time_ns": 10,
        },
        {
            "thread_id": 40,
            "thread_name": "instant",
...
def test_excludes_idle_and_keeps_migration_and_rename_distinct(kat_run):
    ...
    assert {
        row["thread_name"]
        for row in rows
        if row["thread_id"] == 20 and row["cpu"] == 1
    } == {"mover", "renamed-mover"}


def test_zero_complete_non_idle_intervals_has_stable_empty_schema(kat_run):

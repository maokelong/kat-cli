...
def build_sdk_wheel(
    ...
    *,
    ...
    temporary_root = Path(
        tempfile.mkdtemp(prefix="sdk-wheel-build-", dir=output.parent)
    )
    try:
        ...
        subprocess.run(
            command,
            check=True,
            cwd=repository / "kat/sdk",
            env=environment,
        )
        wheels = list(built.glob("kat_sdk-*.whl"))
        if len(wheels) != 1:
            raise ValueError(
                f"expected one SDK wheel, found {len(wheels)}"
            )
        payload_builder.validate_sdk_wheel_archive(
            wheels[0],
        ...
        (artifact / "SHA256SUMS").write_text(checksum.read_text(encoding="ascii"), encoding="ascii")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"SDK wheel build failed: {error}", file=sys.stderr)
        return 1
    return 0

...
#[test]
fn preserves_large_callchain_id_exactly() {
    let directory = tempfile::tempdir().unwrap();
    let database = directory.path().join("trace.db");
    let symbols = directory.path().join("symbols");
    let output = directory.path().join("symbols.xlsx");
    ...
    connection
        .execute_batch(
            "INSERT INTO data_dict (id, data) VALUES (1, '/system/lib/libmissing.so');
             INSERT INTO native_hook_frame
                 (id, callchain_id, depth, symbol_id, file_id, vaddr) VALUES
                 (1, 9007199254740993, 0, NULL, 1, 1);",
        )
        .unwrap();
    ...
    let mut workbook: Xlsx<_> = open_workbook(&output).unwrap();
    let symbols = workbook.worksheet_range("symbols").unwrap();
    let rows = symbols.rows().collect::<Vec<_>>();
    assert_eq!(text(&rows[1][0]), "9007199254740993");
}

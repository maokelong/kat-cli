...
use clap::Parser;
use kat_rs_native_hook_symbolize::{MissingModule, get_symbols};
...
fn run(cli: Cli) -> Result<()> {
    ...
    let inputs = frames
        .iter()
        .map(|frame| frame.input_value.clone())
        .collect::<Vec<_>>();
...
fn read_frames(path: &PathBuf) -> Result<Vec<Frame>> {
    ...
    let mut statement = connection.prepare(
        "SELECT f.callchain_id, f.depth, COALESCE(NULLIF(s.data, ''), m.data || '+0x' || lower(printf('%x', f.vaddr)), '') AS input_value
...
fn write_workbook(
    ...
    for sheet_index in 0..sheet_count {
            ...
            .enumerate()
        {
            ...
            worksheet.write_string(row, 0, frame.callchain_id.to_string())?;
            worksheet.write_number(row, 1, frame.depth as f64)?;
            worksheet.write_string(row, 2, &frame.input_value)?;

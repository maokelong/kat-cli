...
use crate::proto_codegen::{proto_file, snake_to_upper_camel};
...
fn render_decoder(output: &mut String, spec: &FixedResultPluginSpec) {
    ...
    writeln!(
        output,
        "        push_fixed_result_record(sink, FixedResultRecord::{}(Box::new(config)))",
        spec.config_variant()
    )
    .expect("write to string");
    output.push_str("    }\n\n");
    ...
    writeln!(
        output,
        "        push_fixed_result_record(sink, FixedResultRecord::{}(Box::new(result)))",
        spec.result_variant()
    )
    .expect("write to string");
    output.push_str("    }\n");

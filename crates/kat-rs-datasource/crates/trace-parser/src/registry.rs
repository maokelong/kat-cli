...
use crate::{
    options::{
        ...
        PARSE_PHASE_FILE_READ, PARSE_PHASE_UNWRAP,
    },
...
pub fn parse_trace_bytes_with_options(
    ...
    let format = detect_unwrapped_trace_format(&bytes);
    insert_phase(
        &mut phase_elapsed_ms,
    ...
    let mut outcome = match format {
        TraceFormat::Htrace => {
            ...
            ParseOutcome {
                parsed: parser.parse_bytes(&bytes)?,
                phase_elapsed_ms: BTreeMap::new(),
            }
        }
        TraceFormat::BytraceText => {
            let mut parser = BytraceParser::default();
            parser.parse_bytes_with_options(&bytes, options)?
        }
        TraceFormat::RawTrace => {
    ...
    phase_elapsed_ms.extend(outcome.phase_elapsed_ms);
    outcome.phase_elapsed_ms = phase_elapsed_ms;
    Ok(outcome)
}
...
mod tests {
    ...
    fn detects_bytrace_text() {
        assert_eq!(
            detect_trace_format(BYTRACE_SAMPLE),
            TraceFormat::BytraceText
        );
    }

use crate::options::{
    ParseOptions, ParseOutcome, PARSE_PHASE_BUILD_RECORD_BATCHES,
    PARSE_PHASE_BYTRACE_FINISH_INTERVALS, PARSE_PHASE_BYTRACE_PARSE_LINES,
};
...
impl BytraceParser {
    ...
    fn reset_for_input(&mut self, bytes: &[u8]) {
        ...
        *self = Self::new();
        ...
        let estimated_lines = (bytes.len() / 96).max(1024);
        self.tables
            .reserve_bytrace_rows(estimated_lines, self.options.wants_raw_events());
    ...
    fn parse_text(&mut self, text: &str) -> ParseResult<()> {
        for (line_index, line) in text.lines().enumerate() {
            ...
            match parse_bytrace_line(line) {
                ...
                None if self.options.wants_raw_events() => {
    ...
    fn handle_event(&mut self, event: BytraceLine) -> ParseResult<()> {
        ...
        match event.event_name.as_str() {
            "sched_switch" => match parse_sched_switch(&event.args) {
                ...
                None => {
                    if self.options.wants_raw_events() {
            ...
            "tracing_mark_write" | "print" => {
                if !self.options.is_bytrace_scheduler_only() {
    ...
    fn finish_with_metrics(
    ...
    ) -> ParseResult<ParsedTrace> {
        let finish_started = Instant::now();
        self.finish_open_intervals();
        phase_elapsed_ms.insert(
            PARSE_PHASE_BYTRACE_FINISH_INTERVALS.to_string(),
            finish_started.elapsed().as_millis() as u64,
        );
        ...
        phase_elapsed_ms.insert(
            PARSE_PHASE_BUILD_RECORD_BATCHES.to_string(),
    ...
    pub fn parse_bytes_with_options(
        ...
        phase_elapsed_ms.insert(
            PARSE_PHASE_BYTRACE_PARSE_LINES.to_string(),
            parse_started.elapsed().as_millis() as u64,
        );
        let parser = std::mem::take(self);
        let parsed = parser.finish_with_metrics(&mut phase_elapsed_ms)?;
        Ok(ParseOutcome {
            parsed,
            phase_elapsed_ms,
        })
...
impl HarmonyTraceParser for BytraceParser {
    ...
    fn parse_bytes(&mut self, bytes: &[u8]) -> ParseResult<ParsedTrace> {
        Ok(self
            .parse_bytes_with_options(bytes, &ParseOptions::full())?
            .parsed)
    }
}
...
mod tests {
    ...
    #[test]
    fn scheduler_only_parse_keeps_sched_slice_and_skips_raw_event() {
        let line = "          <idle>-0     (-----) [001] d..3 168758.663017: sched_switch: prev_comm=swapper/1 prev_pid=0 prev_prio=120 prev_state=R ==> next_comm=Binder:924_3 next_pid=1200 next_prio=120";
        let options = ParseOptions::for_required_tables(["sched_slice"]);

        let outcome = BytraceParser::default()
            .parse_bytes_with_options(line.as_bytes(), &options)
            .expect("parse fast path");

        assert_eq!(outcome.parsed.tables.sched_slice.num_rows(), 1);
        assert_eq!(outcome.parsed.tables.raw_event.num_rows(), 0);
        assert!(outcome
            .phase_elapsed_ms
            .contains_key("parser.bytrace.parse_lines"));
        assert!(outcome
            .phase_elapsed_ms
            .contains_key("parser.build_record_batches"));
    }
    ...
    fn parses_repository_bytrace_fixture() {
        let fixture = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../../../tests/fixtures/traces/ut_bytrace_input_thread.txt");
        if !fixture.exists() {

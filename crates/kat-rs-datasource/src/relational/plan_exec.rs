...
impl CompiledRootPlan {
    pub(super) fn new(root_message: &str) -> Self {
        let plan_items = expansion_plan_for_roots(&[root_message]);
        let output_tables = plan_items
            .iter()
            .map(|item| item.output_table.clone())
            .collect::<HashSet<_>>();
        let mut items: Vec<_> = plan_items
            .into_iter()
            .map(|item| CompiledPlanItem::new(item, &output_tables))
            .collect();
        let parent_index_tables = items
            .iter()
            .flat_map(|item| item.parent_table_by_segment.iter())
...
impl CompiledPlanItem {
    fn new(item: ExpansionPlanItem, output_tables: &HashSet<String>) -> Self {
        let parent_table_by_segment = item
            .source_path
            .iter()
            .enumerate()
            .map(|(index, _)| {
                let table = table_name(&item.root_message, &item.source_path[..=index]);
                (output_tables.contains(&table) && table != item.output_table).then_some(table)
            })
            .collect();
        Self {
            item,
    ...
    fn optional_child_field(&self) -> Option<(&[String], &str)> {
        ...
        if self.item.rule != ExpansionRule::OneofVariantTable {
            return None;
        }
...
fn compile_oneof_variant_groups(items: &[CompiledPlanItem]) -> Vec<Arc<CompiledOneofVariantGroup>> {
    ...
    for (item_index, item) in items.iter().enumerate() {
        if item.item.rule != ExpansionRule::OneofVariantTable {
            continue;
        }
...
impl CompiledOneofVariantGroup {
    ...
    fn push_variant(
        ...
        if self.parent_table_by_segment.is_empty() {
            self.parent_table_by_segment =
                compiled_item.parent_table_by_segment[..self.parent_path.len()].to_vec();
        }
...
fn items_for_payload<'a>(
    plan: &'a CompiledRootPlan,
...
impl RelationalDatasetSink {
    ...
    fn ensure_root_plan(&mut self, root_message: &str) -> &CompiledRootPlan {
        self.compiled_plans
            .entry(root_message.to_string())
            .or_insert_with(|| CompiledRootPlan::new(root_message))
    }
    ...
    fn emit_item(
        ...
        match item.rule {
            ExpansionRule::RootScalars => self.emit_root_scalars(source_index, payload, item),
            ...
            ExpansionRule::MessageFieldTable => {
                self.emit_message_field_table(source_index, payload, compiled_item)
            }
            ExpansionRule::OneofVariantTable => {
                self.emit_oneof_variant(source_index, payload, compiled_item)
            }
        }
    }

    fn emit_root_scalars(
        &mut self,
        source_index: u64,
        payload: &DecodedPayload,
        item: &ExpansionPlanItem,

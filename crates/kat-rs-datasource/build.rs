...
fn generate_event_family_code(family: &EventFamilySpec) -> std::io::Result<()> {
    ...
    fs::write(
        out_dir.join(family.rows_file),
        render_event_rows(family, &messages),
    )?;
    fs::write(
...
fn render_event_rows(family: &EventFamilySpec, messages: &[ProtoMessage]) -> String {
    ...
    writeln!(output, "pub(crate) struct {} {{", family.meta_name).expect("write to string");
    ...
    output.push_str("}\n\n");
    output.push_str("#[allow(dead_code)]\n");
    writeln!(output, "impl {} {{", family.meta_name).expect("write to string");
    ...
    for message in messages {
        render_row_struct(&mut output, family, message);
    }
...
fn render_event_table_builders(family: &EventFamilySpec, messages: &[ProtoMessage]) -> String {
    ...
    writeln!(output, "        {},", family.meta_name).expect("write to string");
    for message in messages {
...
fn render_row_struct(output: &mut String, family: &EventFamilySpec, message: &ProtoMessage) {
    ...
    writeln!(
        output,
        "    pub(crate) fn new(meta: &{}, message: {}) -> Self {{",
        family.meta_name, message.name
    )
    .expect("write to string");
    output.push_str("        Self {\n");

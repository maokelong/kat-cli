...
本文描述专家总结的 trace 关键路径抽取经验。它面向人类维护，不直接执行 SQL。当前边界是：LLM 只对照 pack/mapping 判断 coverage/gap；可执行步骤由 pack analysis plan 和通过 Cargo 执行的 analyze 子命令提供。
...
`finnal_blocked_caller` 保留专家策略原始字段名；若后续 pack 标准化字段名，应在 mapping 中记录兼容关系。
...
本文只描述专家策略，不描述当前 pack 的实现状态。

可执行覆盖情况、部分覆盖项和能力缺口由 `pack-mappings/` 中的具体映射文档审阅。LLM 不能根据本文直接生成临时 SQL、临时 probe 或手写 evidence；只能通过 `kat-rs-cli analyze` 获得运行产物。

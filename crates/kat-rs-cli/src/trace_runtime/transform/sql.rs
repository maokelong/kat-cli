...
use crate::trace_runtime::{adapter::DatasetAdapter, pack::spec::TransformSpec};
...
fn render_sql(template: &str, params: &Value) -> Result<String> {
    ...
    while let Some(start) = rest.find("${") {
        ...
        rendered.push_str(&sql_literal(params.get(name))?);

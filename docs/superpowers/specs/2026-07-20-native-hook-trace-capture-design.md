...
| 60 秒 | 59.82 秒 | 44,523,108 B | 27,422,720 B | 193,241 | `10000` | 未启用 |
| 120 秒 | 119.82 秒 | 44,362,838 B | 27,185,152 B | 189,179 | `10000` | 未启用 |

三次均完成 trace_streamer 转换。最新 30 秒验证将启动校验移到采集前，在采集窗口内动态识别 18 个按钮并完成 10 次随机点击；随机种子、候选按钮和点击总数均写入 `uitest.log`。数据库 `trace_range` 与各自配置时长一致，未发生采集时长截断。

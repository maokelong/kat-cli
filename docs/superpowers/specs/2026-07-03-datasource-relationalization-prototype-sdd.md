# kat-datasource 关系化转换 Prototype SDD
...
`kat-datasource` 的目标是提供底层 datasource 能力：读取 `.htrace`，decode profiler payload，把 `.proto` 中 SQL 不友好的复杂结构转换成 SQL 友好的关系表，再通过 Parquet + DataFusion 查询。
...
3. 验证 `kat-datasource` 可以把当前 profiler protobuf payload 机械转换成 SQL 友好的关系表。
...
| `import_hitrace` 入口 | 保留公开入口和 `DatasetWriteTarget` 参数，内部使用同一条关系化 materialize 路径。 |
...
```text
kat/platform/datasource/src/formats/hitrace/
```
...
```text
kat/platform/datasource/src/decode/profiler/
```
...
```text
kat/platform/datasource/src/record.rs
```
...
1. 从 `MemoryData` 的 `PayloadValue` tree 开始。
...
```text
kat/platform/datasource/src/relational/
```
...
```text
kat/platform/datasource/src/relational/plan.rs
kat/platform/datasource/src/relational/plan_exec.rs
kat/platform/datasource/src/relational/table_data.rs
```
...
```text
kat/platform/datasource/src/relational/sink.rs
kat/platform/datasource/src/relational/plan_exec.rs
kat/platform/datasource/src/relational/table_data.rs
kat/platform/datasource/src/relational/table_batch.rs
```

职责：

1. 接收 `TraceRecord::DecodedPayload`。
2. 首次遇到一个 `root_message` 时生成并编译 expansion plan，后续同类 payload 复用 `CompiledRootPlan`。
3. 按编译后的 dispatch 顺序遍历 `PayloadValue`，把命中的源字段直接追加到目标表的 Arrow column builders。
4. 维护每张表自己的 `row_index`，并只为确实会被子表引用的父表维护 `parent_index` 索引。
5. 表 buffer 达到阈值时生成 `RecordBatch`，交给单一后台 Parquet writer。
6. finish 时 flush 剩余 buffer，关闭写入 channel，等待后台 writer 完成并返回 `DatasetWriter`。

#### 提前整理展开步骤

descriptor 与转换规则先生成 `ExpansionPlanItem`。第一次遇到某种 `root_message` 时，关系化层把这些 plan item 整理成一个可重复使用的 `CompiledRootPlan`。

例如 `FtraceEvent.event` 是 oneof，一条实际数据只会包含一个 variant：

```text
FtraceEvent.event.sched_switch_format
  -> ftrace_event__sched_switch_format

FtraceEvent.event.cpu_idle_format
  -> ftrace_event__cpu_idle_format
```

`CompiledRootPlan` 会提前建立 variant 与目标表之间的对应关系。处理实际数据时，先读取当前出现的 variant，再直接执行对应表的写入，不需要依次检查其他没有出现的 variants。

整理过程还包括：

1. 预先计算每个 plan item 在各层级可能对应的父表。
2. 把共享同一父 path 的 optional child 检查归组；处理一个 payload 时，同一父 path 只收集一次实际存在的 child field。
3. 缓存目标表列、variant 字段和执行顺序，后续同一 `root_message` 的 payload 直接复用。

这里的“整理”只减少重复的 path 查找、字段存在性检查和 descriptor 解析，不生成新的业务规则。表名、展开规则、行数、父子关系和输入顺序不因此改变。

#### 字段值直接进入列缓冲区

假设关系化层从 `ProcessMemoryInfo` 中读到：
...
```text
pid  -> Int32Builder.append_value(123)
name -> StringBuilder.append_value("app")
vm_rss_kb -> UInt64Builder.append_value(4096)
```

关系化执行不先为每一行构造完整的 `RelationalRow` 或 `Vec<CellValue>`。`table_data.rs` 找到目标 message 后，将普通 scalar、string 和 bytes 等源值直接传给 `table_batch.rs` 中对应类型的 Arrow builder；只有 enum name 等派生值需要单独转换。

每张表的 builders 持续接收多行，达到 flush 条件后一次生成 `RecordBatch`。因此数据路径是“读取一行 -> 逐列追加 -> 批量写出”，同时保留动态 descriptor 驱动的表结构。
...
`row_index` 不能使用当前 buffer 的行数计算。因为 flush 后列缓冲区会清空，同一张表将出现重复行号。每张 `TableBuffer` 独立维护 `next_row_index`：
...
因此无论一张表 flush 多少次，`row_index` 都保持同一张表内唯一递增，`source_index + parent_index -> row_index` 的父子 join 语义不变。

编译 plan 时会找出哪些输出表实际被后代 plan item 当作父表，并只对这些表设置 `needs_parent_index`。不被任何子表引用的叶子表不写入 `parent_indexes`，避免为大量叶子行维护无用的 `ordinals -> row_index` 映射。

`parent_indexes` 只在一个 decoded payload 内使用。例如 `MemoryData.processesinfo.smapinfo` 只会回到当前 `MemoryData` payload 的 `processesinfo`。每处理完一个 `DecodedPayload` 后清空 parent index，避免它随整个 trace 增长。该裁剪只改变内部索引数量，不改变输出的 `row_index`、`parent_index` 或父子 join 结果。
...
ParquetWriteWorker
  sender: bounded FIFO sync_channel(capacity = 0)
  thread: kat-parquet-writer
  writers: table -> DatasetTableWriter

TableBuffer 达到阈值
  -> 生成 Arrow RecordBatch
  -> 封装为 TableChunk 并发送给后台 writer
  -> 重置当前 builders、buffered_rows 和 estimated_bytes

background writer
  -> 按 channel FIFO 顺序接收 TableChunk
  -> 首次看到表时创建 DatasetTableWriter
  -> 后续 chunk 复用同一张表的 writer
...
finish
  ...
  -> 后台线程写完已接收的 chunk
  -> close 每张表 writer
  -> join 后返回 DatasetWriter
```

后台只启动一个 `kat-parquet-writer`，所有表共享这一条写入线程。容量为 `0` 的有界 channel 表示 producer 与 writer 交接 `TableChunk` 时进行 rendezvous：writer 接收当前 chunk 后，关系化线程可以继续构造下一批，Parquet 写入与下一批关系化转换可以重叠；channel 本身不额外积压完整 batch。单 writer 和 FIFO channel 保证 chunk 提交顺序不变，也不允许同一张表并行写入。

writer 线程拥有所有 `DatasetTableWriter`，关系化线程只维护 Arrow builders。写入错误通过 worker 的返回值传回；finish 必须关闭 sender 并 join worker，不能在后台写入尚未完成时提交 dataset。

这里不把一张表拆成多个 catalog entry，也不把 catalog path 改成目录。`DatasetTableWriter` 支持同一张表多次 `write(batch)`，因此既能把内存控制在 `O(所有表当前 buffer 行)`，也不需要同步修改 reader、catalog 校验和 DataFusion 注册逻辑。
...
```text
kat/platform/datasource/src/dataset/
kat/platform/datasource/src/query.rs
```
...
kat/platform/datasource/
  ...
  src/
    ...
    relational/
      ...
      plan_exec.rs                     # 缓存 CompiledRootPlan，共享 path/presence 检查并分发到具体表写入
      table_data.rs                    # 按 source_path 找行数据，并把 message 字段追加为列值
      table_batch.rs                   # 维护 Arrow builders/flush，并通过单后台 worker 顺序写入 Parquet
...
```text
cargo test -p kat-datasource --test hitrace_architecture_contract
cargo test -p kat-datasource --test proto_contract
cargo test -p kat-datasource --test hitrace_datasource_query
cargo test -p kat-datasource --test plugin_flow_contract
...
额外做过源码文本检查，确认旧 profiler decoder 注册入口没有继续出现在 `kat/platform/datasource/src` 中。

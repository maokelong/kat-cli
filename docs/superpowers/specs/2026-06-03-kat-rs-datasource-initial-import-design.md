...
- 建立统一日志约束：诊断信息、内部状态和异常路径使用日志模块，不直接使用 `print` / `println!` / `eprintln!`。
- 建立性能 Trace/metrics 机制：parse、model build、query、CLI 调用链路都需要可度量，从而能定位端到端冷启动的主要耗时。
...
- 提供 datasource 相关子命令。
- 初始化日志配置。
- 分离用户输出和诊断日志：命令结果可以写入 stdout/stderr writer，诊断信息必须走日志模块。
...
- `src/trace.rs`：端到端阶段耗时 Trace，记录 parse/build/query 等关键阶段。
...
CLI、datasource、parser、query 的诊断信息统一使用日志模块控制，避免直接使用 `print` / `println!` / `eprintln!` 输出内部状态。用户可见的命令结果可以通过受控 writer 输出，例如 JSON、表格或错误返回，但这类输出不承担诊断日志职责。

这条约束需要从骨架阶段建立，否则后续 parser/query 接入后很难区分真实命令输出、调试噪声和 CI 日志。后续 PR 需要提供统一日志初始化入口，并在验证中检查 Rust 源码里没有直接诊断输出。

### 7. 性能 Trace 从骨架开始

datasource 的核心目标之一是数据准确且查询极快，因此不能只在发现性能问题后再补观测。后续 PR 需要在 datasource lib 中建立轻量 Trace/metrics 机制，至少覆盖：

- CLI 调用 datasource 的总耗时。
- trace 文件读取和 protobuf htrace parse 耗时。
- trace-model table/RecordBatch build 耗时。
- trace-query SQL 注册和执行耗时。
- validate/inspect/query 等 datasource 对外 API 的阶段耗时。

Trace 结果应当可以进入结构化 metrics 输出，便于端到端冷启动测试比较不同阶段耗时。日志负责说明发生了什么，Trace/metrics 负责说明耗时在哪里，两者边界需要保持清晰。
...
```text
rg "println!|eprintln!|print!" crates
cargo test -p kat-rs-datasource --locked
```

预期结果：
- Rust 生产代码没有直接诊断输出。
- datasource metrics/Trace 测试能够证明阶段耗时被记录。

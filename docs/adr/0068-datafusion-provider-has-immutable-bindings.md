...
`ds.DataFusionProvider(tables=..., catalog=...)` 在构造时复制 `tables` Mapping 的结构并校验 relation 集合，随后不提供 `register()`、`remove()`、`replace()` 或其他可变 catalog API。修改原 Mapping 不改变 Provider；需要更换 relation 集合时，调用方构造新的 Provider。Provider 可以重复调用 `query()`，每次查询都创建独立短命 Session，并在调用开始时取得每张可追加 Table 的当前快照。

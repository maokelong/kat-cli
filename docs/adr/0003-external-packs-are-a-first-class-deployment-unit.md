...
> ADR-0062 与 ADR-0063 将短命 PACK discovery 扩展到 `kat bind`、`kat materialize` 以及可能解析 External Binding 的 `kat query`：这些命令按 manifest identity 选择 PACK，并接受可重复的精确 `--pack-dir` candidates；该路径只作用于本次命令，不写入 Binding。Binding 只保存 PACK identity 与 Source name，不保存 bind 时的 PACK directory、代码、版本或哈希；以后解析 External Binding 或重放 Materialized recipe 时，由当次 discovery 唯一选中的同名 PACK 解释 argv 并取得凭据。`kat inspect --dataset` 仍不 discovery，`kat test` 仍直接接收一个精确 PACK directory。本文其余无 registry、candidate 校验、canonical 去重与同名冲突决定继续有效。

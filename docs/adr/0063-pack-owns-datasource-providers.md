...
Datasource Provider 是 PACK 拥有并由 Workflow 直接 import、构造和调用的普通 Python 类；KAT 不再以 `SourceExecutor`、`ctx.provider()` 和不可替换的 `kat.Provider` facade 包装来源实现。PACK 固定源码布局新增可选顶层 `datasources/`，规范 module identity 为 `kat.pack.datasources.*`；Runtime 只挂载该 namespace 并服从标准 Python import，不扫描、预加载、发现或注册 Provider。KAT 改为通过独立公共模块 `kat.datasource` 提供可组合的 Schema、Table、`write()`、`open()`、Catalog 与具体 `DataFusionProvider` Toolkit，使来源作者可以复用数据面而不把来源语义和生命周期交给平台 facade；这些能力不再平铺到 `kat.*` 顶层。`DataFusionProvider` 是显式构造的本地查询工具，不是 Datasource Provider 基类或 KAT Provider facade。

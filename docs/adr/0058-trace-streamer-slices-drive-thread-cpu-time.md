...
本决定只交付 #122 User Story 21 的预发布联调：`kat-openharmony-demo/thread-cpu-time` 使用 Deprecated Trace Streamer SQLite 输入，验证 Import、Inspect、Run 与有界 Query 的机制链路。它不完成 #141，也不替代 ADR-0048 和 ADR-0024：长期 `kat-kernel/thread-cpu-time` 仍必须以 Hitrace `sched_switch` 为唯一 Required table，按相邻 switch 形成完整可观测区间，并以真实 Hitrace fixture 验收。

Demo 保持无参数入口、唯一 `thread_cpu_time_by_cpu` Output、按线程/名称/CPU 聚合以及稳定排序。它独立承担 Deprecated Trace Streamer 的依赖闭包和退场责任，符合 ADR-0028；不得被描述为正式 `kat-kernel` 能力或长期 Hitrace 用户闭环。
...
首次正式发布前，Trace Streamer Datasource、其 SQLite 读取依赖和 `kat-openharmony-demo` 中依赖它的 Workflow 必须一起删除，或先迁移到已满足 ADR-0048 的 Hitrace facts；不保留 alias、兼容期或把 Demo 提升为 `kat-kernel` 的迁移入口。

...
Windows 载荷的设计目标是不运行 `VC_redist`、不修改系统状态、不要求管理员权限，也不依赖目标机器已经安装系统级 VC Runtime；这些 app-local DLL 与 KAT Skill 其余内容一起原子更新。当前生成流水线的 Windows smoke 运行在预装开发工具和系统级 VC Runtime 的 GitHub 托管 `windows-2025` builder image，只验证最终归档的装配、重定位、Bundled Python 选择及 Import → Run → Query 机制链路，不构成干净客户端依赖闭包验收。Windows 升级为正式支持前，必须按 [Issue #143](https://github.com/maokelong/kat-rs/issues/143) 在未预装额外开发工具、Python 或系统级 VC Runtime 的 Windows 10/11 客户端完成同一闭环；在此之前 Windows 保持预发布候选状态。

...
> ADR-0062 已删除 KAT Data Home 中可被运行隐式选择或由来源操作自动生成的“默认 Dataset”：`kat bind`、`kat materialize` 明确接收目标 Dataset path，`kat run --dataset` 可选但省略时不查找默认位置。本文其余 Skill/Payload 原子发布和可写状态不进入 Skill 的边界继续有效。
...
外层发布生命周期由固定版本的 `dist`（原 cargo-dist）管理，包括版本与 tag、发布计划、custom hook 调度、校验和、托管和 GitHub Release。KAT 关闭 `dist` 的内建 local artifacts；local-artifact adapter 拥有实际原生构建环境，Linux 与 Windows Platform Payload Builder 是 runner、container 和平台工具链的唯一修改入口。`dist` 计划中未执行的原生 runner matrix 不构成 KAT 的有效构建环境合同。Skill Assembly Adapter 只负责 `dist` 不理解的 Skill 路径映射和两个黑盒载荷的汇总；KAT 不手改生成的 release workflow，也不重建发布编排器。发布通道校验、共享输入准备或任一平台构建失败时，不得进入托管。`release/kat/dist.toml` 是 KAT 发布版本入口，发布准备阶段必须校验 Cargo workspace 与 Bundled Python Host 的 package metadata 使用同一版本。
...
Windows 载荷的设计目标是不运行 `VC_redist`、不修改系统状态、不要求管理员权限，也不依赖目标机器已经安装系统级 VC Runtime；这些 app-local DLL 与 KAT Skill 其余内容一起原子更新。当前生成流水线的 Windows smoke 运行在预装开发工具和系统级 VC Runtime 的 GitHub 托管 `windows-2025` builder image，只验证最终归档的装配、重定位、Bundled Python 选择及 Bind/Materialize → Run → Query 机制链路，不构成干净客户端依赖闭包验收。Windows 升级为正式支持前，必须按 [Issue #143](https://github.com/maokelong/kat-cli/issues/143) 在未预装额外开发工具、Python 或系统级 VC Runtime 的 Windows 10/11 客户端完成同一闭环；在此之前 Windows 保持预发布候选状态。

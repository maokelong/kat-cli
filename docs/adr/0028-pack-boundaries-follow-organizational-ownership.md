...
同一 Skill 中原子发布不等于其中每个 PACK 具有相同发布门。预发布 `kat-openharmony-demo` 与同属 Kernel Team 的正式 `kat-kernel` 分离，是因为前者独立承担 Demo 可信度门、Deprecated Trace Streamer 依赖闭包，以及正式发布前必须单独决定迁移、晋升或删除的退场责任；这些是实际维护与发布责任，不是按单个 Workflow 或成熟度随意拆分。

...
CLI 通过锁定 SDK Git 提交和版本构建标准 PEP 517 wheel，Payload Builder 校验 wheel 身份与 SHA-256 后用 pip 安装到 Bundled Python Host。SDK 独立版本化；CLI 更新 SDK 依赖时重新验证组合，不要求 SDK 版本与 CLI 相等。SDK 来源锁、Runtime 依赖和第三方依赖锁必须一致。首次交付使用仓库源码和 wheel，不假定 PyPI 已有同名发布。

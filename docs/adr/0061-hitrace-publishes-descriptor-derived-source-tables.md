...
> ADR-0062 已把 Hitrace descriptor-derived Parser 的所有权迁入对应 PACK Sources；ADR-0063 的首个 `kat-kernel/hitrace` Source 只发布 `clock_domain`、`clock_snapshot` 与按存在性发布的 `sched_switch`，暂不暴露 Native Hook 或其他 descriptor-derived tables。本文其余 descriptor 映射、provenance、关系和来源专属验证合同继续作为后续扩展的验收条件。

...
PACK 可以用普通 Python Provider 显式调用该二进制，并在当前 Workflow 的临时 workspace 中打开生成的 Parquet Catalog。Provider 负责进程调用、必需关系校验、失败清理和 `query() -> ds.Table`，不重新解析文本、不复制 Parquet Schema，也不注册到 KAT。Workflow 只在 Catalog 存活期间查询，最终 eager Table 由 Runtime 发布为 Run Output。

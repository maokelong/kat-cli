...
| 公共函数文档 | 从 kat Skill 导航找到真实 import、签名、语义和示例；wheel 不携带公共库知识目录 |
...
按用户要求，kat 的 references 同时提供 Workflow、Provider 和公共库的选择与使用入口。新增 Workflow 按领域写入 `references/workflows/`，Provider 按名称写入 `references/providers/`，各自维护分类导航；kat-dev-sdk 将这两类介绍作为开发交付项。SDK 内现有 Workflow/Provider Guide 和生成 API 保留供 inspection 使用，Skill 介绍通过命令指向当前安装版本的合同。此切片仅新增文档及开发要求，验证相对链接、成套 Skill 装配及示例与源码合同一致。

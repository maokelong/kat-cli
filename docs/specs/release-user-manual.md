...
第一版安装与启用指引面向 Claude Code。手册统一以 `/kat` 发起分析、PACK 创建维护和报告复核，由总入口路由；其他任务入口仅作可选说明。
...
用户输入尽可能简短，只表达任务、相关数据或目录及用户特有的约束，不重复 Skill 已负责的能力发现、执行检查、测试、证据整理或失败处理要求。创建示例保留具体分析能力与输入，名称、维护方等必要缺项可在对话中补齐，不要求用户一次填完表单。手册正文解释应看到的结果；只有空骨架可被发现，不足以表示一个分析能力已完成，验收仍应核对真实 Workflow 执行与测试结果。
...
结合当前发布目录，手册指导用户将同一版本的 `kat/`、`kat-analyze/`、`kat-author/`、`kat-review/` 四目录一起放入上述 skills 目录，保持同级，再从 Claude Code 调用 `/kat`。必须保留全部 references、脚本与平台载荷，不能只复制 `SKILL.md`。具体平台步骤与首次调用仍须从实际发布包验证，官方加载规则本身不等于 KAT 安装已通过验收。
...
每个主要操作提供可复制并替换实际信息的简短自然语言示例，随后解释应看到什么，以及失败时应补充什么。已在当前对话中的信息不要求重复输入；诊断或修复、已有结果追问或新增执行、永久删除的范围仍要清楚。CLI 命令只在必要的安装、自检或排错位置出现，不成为日常使用主线。公开说明遵循当前结果契约，不展示伪造的业务结论或未实际运行的成功记录。
...
- HTML 静态检查：标签配对、唯一 ID、14 个锚点、22 个链接（含本地引用）有效；20 个示例块，没有脚本、远程加载元素、CSS 外部导入或外部字体。此检查不替代浏览器显示验收。
- 入口与示例修订后，14 个对话示例均使用 `/kat`，最多两行；独立复核确认保留了实际目标、必要材料和操作意图。重新运行 Windows 装配测试，14 项结果 OK，其中 5 项仍因符号链接权限跳过。
- 修订后重新装配的本地候选归档：228,380,762 bytes，SHA-256 `13da76d07e69bdf77d633dce4fc94e133ca410953f15f6c93b3615447f0a31ed`，顶层精确为四个 Skills。Linux 原生文件系统完整解压到隔离项目的 `.claude/skills/` 后，`build/verify_skill_collection.py` 通过；`kat/user-manual.html` 与源码逐字节相同（35,416 bytes，SHA-256 `070cdf72a0f877e62edb7734f7bca99d56e1bfa369a9202ecb2db285edbf5bd8`）。此候选未发布，载荷来源与限制见下节。
- `git diff --check` 通过。

...
def _events_provider(events: Path, labels: Path) -> dp.DataFusionProvider:
    return dp.DataFusionProvider(
        catalog=dp.open(tables={"events": events, "labels": labels}),
    )
...
def test_tables_not_present_in_the_mapping_are_invisible(tmp_path):
    ...
    provider = dp.DataFusionProvider(
        catalog=dp.open(tables={"events": events}),
    )

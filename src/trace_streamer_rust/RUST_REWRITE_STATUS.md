...
- 新 workspace: `kat-rs-pr/src/trace_streamer_rust`。
- 已提交小样例: `test/resource`。
- 本地大样例: `test/local_resource`，由原 C++ TraceStreamer 项目的 `test/resource` 复制而来，已在 `.gitignore` 中排除。
- 本地验证脚本: `scripts/local_validate_all.ps1`。
- 最新报告:

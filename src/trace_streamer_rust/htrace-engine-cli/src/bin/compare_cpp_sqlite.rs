...
async fn aggregate_report(
    conn: &Connection,
    parsed: &htrace_model::ParsedTrace,
...
async fn rust_query_result(parsed: &htrace_model::ParsedTrace, sql: &str) -> Value {
    rust_query(parsed, sql)
        .await
        .unwrap_or_else(|err| json!({ "error": err.to_string(), "sql": sql }))
}

async fn rust_query(parsed: &htrace_model::ParsedTrace, sql: &str) -> Result<Value> {
